 ...
 if(!class_exists('CP_HeartBeat')){
 	class CP_HeartBeat{
		...
		function __construct(){
			...
			add_action( 'wp_enqueue_scripts',  array( $this, 'cp_qbo_heartbeat_enqueue' ) );
		}
		function cp_qbo_heartbeat_received( $response, $data ) {
		  ...
		  //	if(CP()->qbo->sync_price == 'yes' || CP()->qbo->sync_stock == 'yes' || CP()->store_cost == 'yes'){
		  	if( CP()->qbo->consumer_key && CP()->qbo->consumer_secret && CP()->qbo->license && CP()->qbo->license_info->status=="valid"){
		  		if(CP()->qbo->frequency):
		   			$last 		= get_option( 'cp_last_sync' );
					$current 	= current_time('timestamp');
					if( !$last ){
						CP()->cp_queue_inventory();
						update_option( 'cp_last_sync', $current );
					}else{
		   				$diff 	 	= $current - $last;
						if( $diff > CP()->qbo->frequency ){
							CP()->cp_queue_inventory();
							update_option( 'cp_last_sync', $current );
						}
					}
							 ...
							 while ( $query->have_posts() ) : 
								...
								switch ($act) {
									...
									case 'import-inventory':
										$response['sync-inventory'] 		= cartpipe_request('import', 'inventory', $params);
										break; 
									case 'import-images':
										$response['import-images'] 			= cartpipe_request('import', 'images', $params);
										break; 
									case 'export-inventory':
										$response['sync-inventory'] 		= cartpipe_request('export', 'inventory', $params);
										break; 
									case 'sync-inventory':
									...
									case 'check-customer':
										...
										break;
									case 'create-invoice-pdf':
										$response['create-invoice-pdf'] 	= cartpipe_request('get', 'invoice-pdf', $params);
										break;
									case 'create-salesreceipt-pdf':
										$response['create-salesreceipt-pdf'] 	= cartpipe_request('get', 'salesreceipt-pdf', $params);
										break;
									// case 'generate-pdfs':
										 // $response['generate-pdfs'] 	= cartpipe_request('generate', 'pdfs', $params);
								}
							endwhile;
...
// 
		...
		function cp_qbo_heartbeat_enqueue( $hook_suffix ) {
			...
			add_action( 'wp_print_footer_scripts', array($this,'cp_qbo_heartbeat_footer_js'), 20 );
