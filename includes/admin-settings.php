...
class QBO_Admin_Settings {
	...
	public static function get_settings_pages() {
		if ( empty( self::$settings ) ) {
			...
			$settings[] = include( 'settings/class-qbo-pdf.php' );
	...
	public static function output_fields( $options ) {
		...
		foreach ( $options as $value ) {
			...
			switch ( $value['type'] ) {
				...
				case 'select' :
				case 'multiselect' :
					$auto_create 	= isset( $value['create_account']) ? true : false;
					$option_value 	= self::get_option( $value['id'], $value['default'] );
					$dependency 	= isset( $value['dependency'] ) ? $value['dependency'] : '';
						...
						<td class="forminp forminp-<?php echo sanitize_title( $value['type'] ) ?>">
							<?php switch ($wizard) {
								...
								default:?>
								    ...
								    <?php if( $auto_create ){	
								    	printf('%s <i class="grey fa fa-plus-circle"></i>',  $description );
								    	?>
									   <span class="left create-account-wrapper">
									   	<input
											name="<?php echo esc_attr( 'create_account' ); ?>"
											data-account="<?php echo esc_attr( $value['account']);?>"
											value=""
											class="auto-create-field"
											placeholder="Enter the account name"
											class="auto-create"/>
										<a href="#" class="button button-primary create-account"><?php _e('Create Account', 'cartpipe');?></a> 
										</span>
